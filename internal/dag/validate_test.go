...
var structuralCases = map[string][]issueRef{
	...
	"expansion_bad.json": {
		{dag.CodeExpansionFieldInvalid, "expansion.max_added_steps"},
		{dag.CodeExpansionFieldInvalid, "expansion.max_total_steps"},
	...
	"context_bad.json": {
		...
		{dag.CodeContextFieldInvalid, "steps[8].context.budget_tokens"},
		{dag.CodeContextFieldRequired, "steps[9].context.compaction[0].n"},
		...
		{dag.CodeContextFieldInvalid, "steps[19].context.sources[0].tags"},
		{dag.CodeContextFieldInvalid, "steps[20].context.sources[0].role"},
	},
}
