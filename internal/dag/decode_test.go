...
var decodeInvalidCases = map[string][]string{
	...
	"context_unknown_field.json":             {"steps[0].context.srcs: unknown field"},
	"context_bad_kind.json":                  {`steps[0].context.sources[0].kind: unknown context source kind "database"`},
