...
func TestListPlugins(t *testing.T) {
	...
	if len(resp.Plugins) != 16 {
		t.Fatalf("catalog has %d plugins, want the 16 builtins", len(resp.Plugins))
	}
...
func TestListPluginsCoreSet(t *testing.T) {
	...
	if len(resp.Plugins) != 13 {
		t.Fatalf("core catalog has %d plugins, want 13", len(resp.Plugins))
	}
	for _, p := range resp.Plugins {
