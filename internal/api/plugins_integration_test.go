...
func TestListPlugins(t *testing.T) {
	...
	if len(resp.Plugins) != 17 {
		t.Fatalf("catalog has %d plugins, want the 17 builtins", len(resp.Plugins))
	}
...
func TestListPluginsCoreSet(t *testing.T) {
	...
	if len(resp.Plugins) != 14 {
		t.Fatalf("core catalog has %d plugins, want 14", len(resp.Plugins))
	}
	for _, p := range resp.Plugins {
