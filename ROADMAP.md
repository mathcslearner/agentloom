...
**Depends on:** 7.2, 7.3
Provisioned-as-code dashboards: **Engine** (throughput, queue/PEL/delayed depth, scheduling latency, step latency p50/p95/p99, error/DLQ/reclaim rates) and **API** (RPS, latency, 429s, in-flight). Example alert rules (queue depth growing 10m, DLQ rate spike, reclaim spike, outbox lag). Screenshots into docs.
**Done when:**
- [ ] Dashboards auto-provision in compose; panels render non-empty under the chaos suite
- [ ] Alert rules load in Prometheus; one rule test-fired and documented
- [ ] `docs/observability.md` explains the dashboards and key signals
...
**Depends on:** 8.1, 1.5
Render step config/inputs with references to upstream outputs and run params — `${{ steps.<id>.output.<path> }}`, `${{ run.params.<key> }}` — via Go `text/template` with a restricted FuncMap (get, default, toJson, truncate; no arbitrary code) + JSON-path resolution. Static lint at definition-validation time (referenced step exists and is upstream); strict missing-reference errors at runtime.
**Done when:**
- [ ] Template suite: nested paths, defaults, missing-ref → typed error, injection attempts inert
- [ ] Validation flags references to non-existent or non-upstream steps
- [ ] `echo` pipeline fixture proves multi-hop data flow end-to-end
...
**Depends on:** 8.3
OpenAI implementation of the same interface; provider registry routes by model ID prefix/explicit provider field; identical error-taxonomy mapping and usage extraction; same fixture testing pattern.
**Done when:**
- [ ] Routing table tested (model → provider; unknown model → typed error)
- [ ] Fixture tests mirror 8.3's coverage for OpenAI response/error shapes
- [ ] Providers configurable independently (either can be absent without breaking startup)

#### 8.5 — Mock/simulation provider
**Depends on:** 8.3
Deterministic provider for tests and load: scripted responses (match on prompt substring/regex or call sequence), configurable latency distributions, token counts, failure/429 injection rates. Registered like any provider (`model: "mock/..."`). This is the load-test workhorse (M19) — cheap, deterministic, offline.
**Done when:**
- [ ] Scripting covers: fixed response, sequence, conditional-on-prompt, injected errors/latency
- [ ] Deterministic under seed; zero external calls (asserted)
- [ ] Reused to convert one M4 e2e fixture into an `llm`-step fixture
...
**Depends on:** 8.1, 5.5
Tool interface (name, JSON-Schema args, `Invoke(ctx, args) (result, error)`, capability flags) + `tool` step executor (args via templating, schema-validated before invoke). Built-ins: `http_request` (method/URL/headers/body, **URL allowlist**, timeout, automatic `Idempotency-Key` header from 5.5 for non-GET), `json_transform` (gojq expression). 
**Done when:**
- [ ] Args validated against tool schema pre-invocation (bad args → permanent failure, no call)
- [ ] `http_request` proves idempotency-key reuse across retries against httptest server
- [ ] Allowlist blocks non-approved hosts (SSRF guard) with a typed error
...
**Done when:**
- [ ] Reference retriever ingests and ranks against seeded corpus (integration test)
- [ ] `retrieve` step output shape documented and consumed by an `llm` step in the fixture
- [ ] SPI docs include a "writing a retriever plugin" walkthrough stub
...
**Depends on:** 6.3, 8.3
**ADR-010:** named resources (`anthropic:<model>`, `openai:<model>`, custom tool resources), dual buckets per resource (requests + estimated tokens), backpressure semantics — deny → attempt outcome `throttled` (not a failure), delayed requeue at `retry_after` + jitter, never busy-wait a worker slot; fairness note (per-run throttle cap to prevent one run starving the fleet — documented, implemented if load tests demand). Config loading for resource limits.
**Done when:**
- [ ] ADR covers dual buckets, requeue math, and fairness stance
- [ ] Resource config parsed/validated with tests; unknown-resource policy decided
- [ ] `throttled` outcome added to taxonomy (ADR-006 cross-updated)

#### 9.2 — Limiter middleware for executors
**Depends on:** 9.1
Executor middleware acquiring (request-bucket 1, token-bucket estimated cost) before provider calls; on deny: record `throttled`, schedule delayed redelivery, release the worker (no attempt consumed). Rough token estimator now (chars/4 + declared `max_tokens`); refined by M12's counters. Metrics: throttle count, queue-wait time by resource.
**Done when:**
- [ ] Multi-worker integration: 4 workers vs R=10/min limit → mock provider sees ≤10±ε calls/min
- [ ] Throttled steps eventually complete; attempt counter unchanged by throttles
- [ ] Worker slot released immediately on throttle (throughput of other runs unaffected — asserted)

#### 9.3 — Token-cost reconciliation
**Depends on:** 9.2
Post-call reconciliation: debit/refund the token bucket by (actual usage − estimate) so sustained error in the estimator can't let the fleet exceed provider token budgets. Property test: long-run conservation of bucket accounting.
**Done when:**
- [ ] Over- and under-estimates reconciled on the bucket (unit + property tests)
- [ ] Sustained load with biased estimator stays within configured tokens/min (integration)
- [ ] Reconciliation metrics exposed (estimate error histogram)
...
**Depends on:** 9.4, 9.2
Read-through/write-through middleware ahead of the rate limiter (hits skip limiter and provider entirely); value = output + usage snapshot; attempt records `cache_hit=true` with zero incremental cost + counterfactual "would-have-cost" usage. Metrics: hit/miss/bypass/store by plugin.
**Done when:**
- [ ] E2E: identical `temperature=0` mock-LLM steps → second is a hit (no provider call, no limiter acquire)
- [ ] `temperature>0` bypasses by default; `read_write` opt-in overrides (both asserted)
- [ ] Hit recorded on the attempt with counterfactual usage for M10
...
**Done when:**
- [ ] Bust removes matching keys without blocking Redis (verified under load)
- [ ] Admin-scope enforced; action audit-logged with actor key ID
- [ ] Stats endpoint matches Prometheus counters in an integration test
...
**Depends on:** 8.6
**ADR-012:** attribution rules (LLM usage × price; priced tools; cache hits = $0 actual + counterfactual saved; validation/judge and summarization overhead attributed to their step, flagged as overhead), estimation approach for pre-flight checks, budget semantics (hard vs soft), unknown-model policy (configurable: conservative default estimate + warning event, or fail-closed). Versioned pricing catalog: embedded defaults + file/env override, effective-dated entries ($/1M input & output tokens per model).
**Done when:**
- [ ] Catalog loads/validates; effective-date selection tested
- [ ] Unknown-model policy implemented per ADR with warning event
- [ ] Attribution rules enumerated in ADR with worked examples

#### 10.2 — Cost ledger & aggregation
**Depends on:** 10.1
`cost_ledger` rows per attempt (usage, rate snapshot, computed cost, overhead flag) written in the attempt-completion transaction; run-level aggregates (`spent_usd`, by-model/by-step breakdowns) updated atomically in the same tx; `GET /v1/runs/{id}` includes cost summary; `GET /v1/runs/{id}/cost` full breakdown.
**Done when:**
- [ ] Property test: concurrent attempt completions → aggregate equals sum of ledger exactly
- [ ] Cache hits ledger $0 with counterfactual field populated
- [ ] Cost API returns per-step and per-model breakdowns (contract test)

#### 10.3 — Budget enforcement at claim time
**Depends on:** 10.2, 5.6
Budgets: run `budget_usd`, step `max_usd`/`max_tokens`. Claim path evaluates projected spend (spent + pre-flight estimate); post-attempt re-evaluates actuals. Actions per policy: `proceed`, `downgrade` (10.4), `park` (reason `budget_exceeded`, resumable), `fail`. `PATCH /v1/runs/{id}/budget` raises budget; unpark resumes.
**Done when:**
- [ ] Run parks within one step of its cap under mock pricing (test tolerance defined)
- [ ] `budget_exceeded` event emitted with projection details; API raise → unpark → completes
- [ ] Step-level `max_tokens` enforced pre-flight (oversized request never sent)

#### 10.4 — Model downgrade chains
**Depends on:** 10.3
Step config `model_fallbacks: [...]`: budget-threshold triggers (e.g., >80% of run budget → next cheaper model) evaluated at claim; the actually-used model recorded on the attempt and priced correctly; downgrade emits an event; interacts correctly with cache keys (different model ⇒ different key — asserted).
**Done when:**
- [ ] E2E: expensive→cheap downgrade fires at threshold; ledger prices actual model
- [ ] Downgrade event carries from/to models + trigger reason
- [ ] Exhausted fallback chain falls through to the configured budget action

#### 10.5 — Cost metrics & events
**Depends on:** 10.2, 7.2
Prometheus: cost counters by model/plugin (bounded labels), budget-park counter, saved-by-cache counter. `cost_updated` events appended per attempt completion (feeds the M18 live meter via M16). Grafana cost panel added.
**Done when:**
- [ ] Metrics visible under chaos-suite load; cardinality within ADR-008 budget
- [ ] `cost_updated` events in the run event feed with monotonic run totals
- [ ] Grafana panel shows spend rate + saved-by-cache
