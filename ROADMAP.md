...
**Done when:**
- [x] Every built-in plugin's config editable via generated forms (no hardcoded per-plugin forms) *(`SchemaForm` over `fields.ts`, driven by each plugin's `config_schema` from `GET /v1/plugins` with a published-schema fallback; a vitest asserts a control for every property of all 17 executor configs, and the round-trip into the store)*
- [x] Invalid config marks the node and blocks submit; errors match backend codes on round-trip *(the client validator lives in `@agentloom/graphdef` and reports the backend's `config_field_*` codes/paths, proven against the Go verdict golden `internal/dag/testdata/verdicts.golden.json`; invalid nodes get a destructive ring + count, `useHasBlockingErrors` is the 17.6 submit gate, `mapIssuesToNodes` maps a server 400's issues onto nodes by path)*
- [x] Autocomplete offers exactly the upstream steps' output paths (test with branching graph) *(`refs.upstreamStepIds` mirrors `classifyUpstreamRef`/`Graph.Ancestors` — normal edges only, self/loop excluded; unit + e2e tests over a branching graph)*

*(As built: **entirely under `web/` plus two tiny spec/schema touches** — no Go runtime change, no migration, no config var, no metric. **(1) Backend consumability:** `PluginInfo.config_schema` gained `additionalProperties: true` (so `openapi-typescript` renders it as `{[k]: unknown}`, not the unusable `Record<string, never>`), and `OutputFormatType`/`OutputFormatMode` gained `JSONSchema()` enum methods (the last two closed vocabularies in `internal/dag` without one) so `output_format.type`/`.mode` render as selects. A new **Go golden**, `internal/dag/TestVerdictsGolden`, emits `internal/dag/testdata/verdicts.golden.json` — the exact Decode+Validate verdict for the whole corpus (131 fixtures) — the ground truth the client parity test compares against. **(2) `@agentloom/graphdef` gained a client validator** (`validate/config.ts` + `schema/schema.ts`, a JSON-Schema-subset shape checker): `validateStepConfigs(def, schemas)` mirrors the backend's single-step-local config rules (required fields, `prompt` xor `messages`, `output_format`, `human_approval`, join mode), reporting the backend's codes/paths/messages. The generator also emits `generated/definition.schema.ts` (`DEFINITION_SCHEMA`); `fallbackConfigSchemas()` derives per-step-type schemas from it (the offline source). `config-validate.test.ts` proves parity vs the golden (config-field code+path equality on decode-clean fixtures, reject on decode-fail config fixtures, no false positives on the example corpus; agent-merge + expansion-cap fixtures explicitly deferred to 17.5). **(3) The app** (`web/app`): a `catalog-store` fetches `GET /v1/plugins` (proxy) and layers live executor schemas over the fallback; `ConfigPanel` replaces the Inspector's selection section with a schema-driven `SchemaForm` (widget per field via `fields.ts` + `hints.ts` — model picker, tool/retriever/agent/template picker, prompt editor, enum select, number/boolean, string/object lists, JSON fallback), a per-step Problems list, and a raw-JSON Envelope editor; `PromptEditor` offers upstream-only `${{ }}` autocomplete (`refs.ts`); `useProblems` + a `ProblemsProvider` context mark invalid nodes (O(n)) and drive a toolbar error count; store gained `patchStep`/`setStepEnvelope`. Standard shadcn tokens (`input`/`accent`/`destructive`) + primitives (`input`/`textarea`/`label`/`select`/`checkbox`) added. **Decisions:** the validator lives in `graphdef` (pure, parity-tested against the Go golden), not the app; the shape checker skips enums (config rules own them per field, since join `mode` is a codeless decode error but `output_format.type` a coded validate error); "secret-ref" **not built** (no backend concept; storing a secret in the definition would be a leak); the model picker offers configured-provider-grouped suggestions and *warns* (never errors) on an unroutable bare model; envelope forms stay raw-JSON (schema-driven envelope forms are 17.5). **Accepted residuals:** cross-section/cross-field config rules (agent role merge, run expansion caps) deferred to 17.5; envelope-block bound checks and full corpus accept/reject parity are 17.5; the submit round-trip surfacing of server issues is wired (`mapIssuesToNodes`) but the submit button is 17.6. **Also fixed a pre-existing 17.3 builder e2e flake** (overlapping cascade-placed nodes made two `builder.spec` tests fail headlessly — confirmed identical on the pristine 17.3 commit; spread the nodes / delete the clickable top node). Tests: graphdef vitest 448 (117 parity + 12 units + existing), app vitest 124 (fields/refs/plugins/problems/config-panel/prompt-editor + existing), Playwright `config-panel.spec` (2) + the repaired `builder.spec` (3) green; workspace lint/typecheck/build green; `make generate` + `make openapi-lint` (100/100) clean. ADR-019 §"Schema-driven config panels (as built, 17.4)"; `web/app/README.md` + `web/lib/graphdef/README.md`. ROADMAP 17.4 ✅. **Next: 17.5** (client-side graph validation — full corpus parity).)*
...
**Done when:**
- [x] Parity test: backend valid/invalid fixture corpus → identical accept/reject verdicts client-side *(`graphdef/test/parity.test.ts` over the Go golden `internal/dag/testdata/verdicts.golden.json` — 131 fixtures — asserts identical accept/reject over every fixture PLUS exact (severity, code, path) equality over the validate stage; the two 17.4 `DEFERRED_TO_17_5` fixtures now pass)*
- [x] Loop-edge authoring flow validates (marked loop OK; unmarked cycle rejected with path highlight) *(the ported cycle detector reports the closing edge + the cycle path as `related[]`; `e2e/validation.spec.ts` draws an unmarked back-edge → `cycle_detected` with both nodes + the edge highlighted, then marks it a loop via the `EdgePanel` → the error clears)*
- [x] Problems panel focuses the offending node/edge on click (e2e) *(`ProblemsPanel` click → `focusIssue` selects the element the issue's own path points at and `fitView`s to it; `e2e/validation.spec.ts` DoD-3 asserts the node is selected + highlighted)*

*(As built: **entirely under `web/`** — no Go runtime change, no migration, no config var, no metric, and no new Go corpus fixture (the port is proven against the existing golden). **(1) The full validator** (`web/lib/graphdef/src/validate`): `validateDefinition(value)` ports `internal/dag` Validate — a **decode stage** (`decode.ts`, a schema-driven walker over the published definition schema reproducing the strict codec's shape/enum/required findings as codeless issues, with the `schema_version` short-circuit) that, when clean, gates a **validate stage** (`definition.ts`) running every rule in the backend's order behind the same well-formed-graph gate, including the graph-semantic checks (a port of the iterative three-colour cycle DFS in `graph.ts` reporting the closing edge + cycle path, loop ancestry, step_output/template ancestry). Two rules the client cannot reproduce exactly are approximated **leniently** (never over-rejecting): a CEL checker (`cel.ts`, tokenizer + Pratt parser + coarse type inference → `invalid_expression`/`expression_not_boolean`, unrecognised → `dyn`) and a ref-level template lint (`template.ts`). Grammar (`grammar.ts`) and bounds (`limits.ts`) are ported verbatim with Go sources cited. `config.ts` became a thin config-scoped façade over the full validator (17.4 API preserved). **(2) Parity** (`test/parity.test.ts`): identical accept/reject over all 131 golden fixtures + exact (severity, code, path) over the validate stage; `cel.test.ts` + `validate.test.ts` add focused tables. **(3) The app:** `useProblems` runs `validateDefinition` and maps issues onto nodes AND edges by index; a hoisted `ProblemsProvider` (inside `ReactFlowProvider`) runs one validation and provides per-element error counts, highlight state, and `focusIssue`; a `ProblemsPanel` (errors-first, click-to-focus + hover-preview) marks the offending element and `fitView`s to it; edge renderers gained invalid/highlight strokes; a minimal `EdgePanel` (loop-edge inspector pulled forward from 17.6) with `store.patchEdge`/`store.selectOnly` makes the loop-authoring flow completable; the toolbar shows `N errors · M warnings` and disables Submit while errors block it. Client-only `advisory_*` budget warnings surface sanity nudges the backend does not model. **Decisions:** validator in `graphdef` (pure, parity-tested); lenient CEL/template checkers; `focusIssue` selects by the issue's own path; minimal edge inspector early for DoD-2. **Accepted residuals:** loop-edge authoring polish + import/export/save/submit are 17.6. Tests: graphdef vitest 778, app vitest 136, Playwright `validation.spec` (2) + existing (8) green; workspace lint/typecheck/build green. ADR-019 §"Client-side graph validation (as built, 17.5)". **Next: 17.6** (import/export, save & submit flows).)*
...
**Done when:**
- [x] E2E: import flagship example → edit a prompt → save new version → submit → run ID returned (mock backend) *(`e2e/save-submit.spec.ts` drives import → edit `field-prompt` → Save v1 → Save v2 → a concurrent out-of-band append → version-conflict guard → Save anyway → Submit params modal → run-id UUID, green against compose; import + byte-export of the flagship is proven at the unit level in `web/lib/graphdef/test/canonical.test.ts`)*
- [x] Export equals canonical backend serialization byte-for-byte (fixture assertion) *(`@agentloom/graphdef`'s `canonicalize` mirrors `dag.Encode`, pinned against the new Go golden `internal/dag/testdata/canonical.golden.json` over the whole corpus, two ways — `canonicalize(parse)` and `canonicalize(toDefinition(toFlow(parse)))`; wired through the DOM in `e2e/import-export.spec.ts` which compares the downloaded bytes to the golden)*
- [x] Unsaved-changes guard and version-conflict (stale save) handling tested *(the `If-Match` precondition on `POST /v1/definitions/{name}/versions` → `409 version_conflict`, covered by store + api Go integration tests, the client `persistence.test.ts`, and the save-submit e2e; the `NavigationGuard` + dirty flag covered by `persistence-state.test.ts` and the import-export e2e)*
...
**Done when:**
- [x] Playwright: submitted run appears and updates in the list without refresh *(`e2e/dashboard-runs.spec.ts` — submit after page load, the row is discovered via `run_created` and flips running → succeeded live, green against compose)*
- [x] Reconnect mid-run resumes with no visual gaps (seq-resume exercised through the UI) *(`e2e/dashboard-run-detail.spec.ts` drops the run WS once mid-run via `page.routeWebSocket`; the connection pill shows a reconnect, the run reaches terminal live, and the assembled timeline's `data-seq`s are exactly `1..N` — gap-free, dup-free)*
- [x] Timeline strip renders the normalized event feed with type filtering *(the `Timeline` strip filters by event category; unit-tested in `test/dashboard/timeline.test.tsx` and e2e-asserted in `dashboard-run-detail.spec.ts`)*

*(As built: **entirely under `web/app`** plus two additive Go facilities (ADR-018) — no migration, no new metric. **(1) Backend:** `RunView.event_seq` (= `runs.next_seq` = `max(events.seq)`, no new column/read) is the dashboard's as-of/resume cursor on both the REST run body and the WS snapshot; the cross-origin `AGENTLOOM_API_WS_ORIGINS` knob → `config.APIConfig.WSOrigins` → `WSOptions.OriginPatterns` on both `websocket.Accept` calls (empty = same-host-only, the 16.3/16.4 default), because a Next.js proxy cannot forward a WS upgrade so the browser dials the API directly — the key never rides the upgrade (a proxy-minted ws-ticket does). OpenAPI + api-client regenerated; `TestWSOriginPatterns`/`TestWSSnapshotEventSeq` integration tests. **(2) Pure reducers** (`src/lib/pure/dashboard/`, no-React boundary): `events.ts` (category + one-line summary, switch **exhaustive over `EventType`**), `run-state.ts` (snapshot+events → `{run, steps: Map, asOf}`, detail counters **derived from the step map**, injected steps from `graph_expanded`, `seq > asOf` idempotence), `run-list.ts` (list-row fold — counters **incremented** but safe under the per-row `event_seq` guard — plus filter predicates + URL codec). **(3) Stream glue** (`src/lib/dashboard/`): `RunController` (injected `RunStream` factory; snapshot shows immediately, backfill applied in one batch at `caught_up`, live events individually; timeline gap-free/dup-free across reconnects) + `useRunController`/`useRunListLive`/`useDefinitionLabels`; a `RuntimeConfigProvider` carries the public `apiPublicUrl`. **(4) Pages:** `/runs` (live chips, URL-synced status/definition/time filters, connection pill, row links) and `/runs/{id}` (a full-bleed `(dashboard)` route group — status-badged steps pane, basic inspector, collapsible timeline strip). **Decisions:** browser dials the WS directly with an origin allowlist; `event_seq` the shared as-of/resume cursor; detail counters map-derived vs list counters event-sourced under the seq guard; graph/inspector panes honest placeholders (18.2/18.3 replace them — the live step map + selection are already wired). Tests: Go config/api unit + `TestWS*` integration green; app vitest 154 → 178 (+24) (dashboard reducers/controller/timeline), runs-page test updated; Playwright 17 (3 new dashboard specs) green against compose. ADR-018 §"Dashboard scaffolding (as built, 18.1)"; ADR-019 §"Live dashboard scaffolding (as built, 18.1)"; `docs/api.md`/`web/app/README.md`. **Next: 18.2** (live DAG view with dynamic expansion).)*
...
**Done when:**
- [x] Every status skin driven by live events (scripted fixture walks all states) *(`skins.ts` maps status → skin exhaustively over `DisplayStepStatus`; `skins.test.ts` walks a scripted event sequence through the full lifecycle and asserts the skin at each state; `dashboard-graph.spec.ts`'s retry+skip test drives retrying→running→succeeded and a skipped branch live on the canvas)*
...
**Done when:**
- [x] E2E: budgeted run climbs the meter, parks at cap, user raises budget, resumes — all live *(`e2e/dashboard-cost.spec.ts` DoD-1: a tight-budget mock chain climbs `data-spent-nano`, flips to `parked` with the parked banner + `data-tier="exceeded"`, then Raise → $1 + resume → `succeeded`, all live)*
- [x] Downgrade banner names from/to models and trigger *(`banner-downgrade` carries `data-from`/`data-to`/`data-trigger`; the e2e's soft-threshold `model_fallbacks` run asserts `mock/sim-1 → mock/cheap`, trigger `budget_threshold`)*
- [x] Meter total matches `GET /v1/runs/{id}/cost` at completion (consistency assertion) *(the DoD-1 e2e asserts `data-spent-nano === summary.spent_nano_usd`; `foldCostEvents` proves the same at the reducer level against the Go cost golden)*

*(As built: **entirely under `web/app`** — no Go change, no migration, no config var, no metric. The meter is stateless off the event feed: `cost_updated` (running totals, non-decreasing in seq — 10.5) and `run_budget_updated` (the budget total, on its payload) are folded into `run.cost` by the 18.1 run-state reducer under the `asOf` seq guard, so the header updates live with no cost refetch. **(1) `run-state.ts` additions:** `cost_updated` now also carries the run budget when present; `run_budget_updated` sets it (correcting the 18.1 placeholder that assumed the budget wasn't on the payload — it is); `mergeRunResponse` folds the body's run-level `cost`/`status`/`park_reason` when the body is ≥ `asOf` (so a raise/unpark refetch reconciles, a stale body never regresses). **(2) Pure derivations** (`pure/dashboard/`, no-React boundary): `cost-meter.ts` (`deriveCostMeter` → a `BudgetTier` `unbudgeted`/`ok`/`warn`/`danger`/`exceeded` at 75/90/100%, `exceeded` also when parked-for-budget; `foldCostEvents` proves convergence to the cost summary — DoD-3 pre-paid) and `budget-banners.ts` (`deriveBudgetBanners` projects `model_downgraded`/`budget_exceeded`/`run_budget_updated` onto seq-keyed banners deduped across a re-backfill, downgrade banners carrying from/to + a resolved trigger label; a live "parked at cap" affordance pinned first while `status==="parked" && park_reason==="budget_exceeded"`, gone on unpark, carrying the Raise action). **(3) Actions** (`useBudgetActions.ts` + `streams.ts` `setRunBudget`/`unparkRun`): raise = `PATCH .../budget` then optional `POST .../unpark` (the ADR-012 resume path); park→resume reflected live via events, so no optimistic state; a 409 on the unpark (already resumed) is tolerated. **(4) Components:** `CostMeter` (header ticker + `role="progressbar"` budget bar + saved-by-cache indicator), `BudgetBanners` (dismissible stack), `RaiseBudgetDialog` (prefilled USD input, warns-not-blocks on a sub-spend budget, resume checkbox defaulting on when parked for budget) — replacing the 18.1 ad-hoc `$` span. **Decisions:** stateless meter (the stream totals are authoritative and monotonic); raise + optional unpark in one dialog; warn-not-block; no backend change (every fact was already on the feed and the REST budget PATCH). **Accepted residual:** the e2e budget arithmetic is calibrated to the offline mock pricing (`mock:*` = $1/$2 per Mtok) with response caching disabled per step (the global cache would serve a repeat run for $0). Tests: app vitest 248 → 277 (+29; `cost-meter`/`budget-banners` pure + render, `raise-budget-dialog`, `run-state`/`run-state-merge` extensions); lint/typecheck/build green; Playwright `dashboard-cost.spec.ts` (DoD-1 climb→park→raise→resume, DoD-2 downgrade banner, DoD-3 consistency). ADR-018 §"Live cost meter & budget UX (as built, 18.4)"; ADR-019 §"Live cost meter & budget UX (as built, 18.4)"; `web/app/README.md`. ROADMAP 18.4 ✅. **Next: 18.5** (HITL approval inbox & decision UI).)*
...
**Done when:**
- [x] E2E: a gate pauses → approve with edit via UI → run continues; edited payload visible downstream *(`e2e/dashboard-approvals.spec.ts` DoD-1: a `human_approval` gate (the canonical `approval_gate.json` shape, offline mock) parks → the node "Decide" affordance opens the dialog → edit the payload text → approve → the run reaches `succeeded` and `publish`'s output carries the edited text, cross-checked against `GET /v1/runs/{id}`. The flagship's own gate is covered by the Go `TestFlagshipResearchCriticWriter`; the simpler gate exercises the identical UI path deterministically)*
- [x] Concurrent decision from another session → this session surfaces the 409 gracefully *(DoD-2: the dialog opens, a second `APIRequestContext` decides the approval via raw REST, and this session's approve reveals the read-only "decided in another session" state — from the 409 `approval_not_pending` on submit or the live `approval_decided` event, whichever wins; unit-covered in `decision-dialog.test.tsx`)*
- [x] Reject path routes per config and renders the outcome *(DoD-3: `on_reject: route` — reject with a comment → `notify_rejected` runs, `publish` is skipped, and the Approval inspector tab shows "rejected → routed to notify_rejected". The reject-plan hint in the dialog names the target before the click, derived from the gate config + graph edge markers)*

*(As built: **entirely under `web/app` plus two additive, non-runtime backend touches** — no migration, no config var, no metric, no OpenAPI change. The 15.3/15.4 approval API (`GET /v1/approvals`, `POST /v1/approvals/{id}:decide`) and the six `approval_*` events were already complete; the generated api-client types already carried the operations/schemas, so **no regeneration**. The two backend touches: (a) a **proxy fix** — the same-origin Next.js proxy joined path segments with `encodeURIComponent`, turning `approvals/<id>:decide` into `…%3Adecide`, which chi (routing on the raw path with `:` as its param delimiter) 404s before the handler; the proxy now preserves a literal `:` verb suffix (`encodeSegment` splits on `:`), with a `proxy.test.ts` case pinning the upstream URL; (b) an **exported Go golden** `internal/api/testdata/approval_list_fixture.json` (`TestApprovalListFixtureGolden`, `UPDATE_GOLDEN=1`, the 13.6/18.3 precedent) — the exact `GET /v1/approvals` wire shape covering pending / approved-with-edit / rejected / expired / **park-expired** (`on_timeout: park` — `expired_at` stamped but status still `pending`, so still decidable) rows, which the frontend tests read as ground truth. **(1) api-client:** four ergonomic re-exports (`ApprovalView`/`ApprovalListResponse`/`DecideApprovalRequest`/`DecideApprovalResponse`). **(2) Pure layer** (`src/lib/pure/dashboard/`, no-React boundary, vitest-tested against the golden): `approvals.ts` (`ApprovalRecord` = the view + a per-record `lastSeq` cursor + `partial` flag; `applyApprovalEvent` folds the four `approval_*` events absolutely by status under the seq guard — a `requested` seeds a *partial* record since the event carries no payload/edit_schema, an `expired` with `action: run_parked` stays pending; `mergeApprovalViews` completes a partial from the authoritative run body without regressing a fresher live record; `decidable`/`parkExpired`/`pendingForStep`/`recordsForStep` selectors), `approval-aging.ts` (`deriveAging` fresh/aging/stale at 1h/24h, `deriveDeadline` countdown/overdue, injected `now`), `decision-outcome.ts` (`rejectPlan` fail-vs-route + targets from the gate config's `on_reject` and the topology's reject-marked edges; `decisionOutcome` rendered text), `edit-validate.ts` (a small CSP-safe JSON-Schema *pre-check* mirroring JSON-Schema **validator** semantics — enforces `required`/`enum`/`type`, allows extra props unless `additionalProperties:false` — NOT the strict Go decoder / graphdef `checkShape`; the server 422 `issues[]` stay authoritative), `approval-list.ts` (inbox row model + `applyInboxEvent` + status/run filter codec, oldest-first). `run-state.ts` gained `RunState.approvals` folded from both the snapshot body and the live feed; `projection.ts` carries a `pendingApprovalId` on a gate node so the DAG can render a Decide affordance. **(3) Glue** (`src/lib/dashboard/`): `streams.ts` `listApprovals`/`decideApproval` (typed `DecisionOutcome` distinguishing 409 `not_pending` and 422 `invalid`+issues)/`refetchApproval`; `useApprovalActions` (drives the decide, classifies the outcome, holds no optimistic state); `useApprovalInbox` (the `useRunListLive` mirror — firehose over the approval events, run-scoped filter, discovery of an unseen gate by re-listing its run); `RunController` refetches the run body on a live `approval_requested` to fill a partial record. **(4) Components:** `DecisionDialog` (payload viewer + optional schema-validated JSON editor gated on `allow_edit`, comment, approve/reject with the reject-plan hint, client + server issue lists, and the **409-recovery read-only "decided elsewhere" state**), the `/approvals` inbox page (`ApprovalsTable` with aging chips, deadline countdown, editable badge, decided-outcome subtext, status+run filters URL-synced, keyset paging, live pill), `ApprovalBanner` (the run-header "N waiting on you" affordance beside the 18.4 budget banners), a `RunStepNode` footer Decide button (stopPropagation, injected via node data like `RunGroupNode.onToggle`), and an InspectorPane **Approval** tab. Plus a `destructive` Button variant. **Decisions:** apply the decide *response* immediately (it carries the decided approval) rather than a pre-response guess — no rollback logic; edit validation is a client pre-check, the server 422 authoritative; inbox discovery via `?run_id=` (no `GET /v1/approvals/{id}` exists); the reject-outcome hint derived from config + edge markers (zero backend change). **Accepted residuals:** no nav-wide pending badge (inbox-page count only); a per-field structured edit form stays deferred (raw JSON editor with pointer-keyed issues); scope-gated hiding of the Decide control is 18.6's rendered-permissions concern (18.5 surfaces a 403 inline). Tests: app vitest **277 → 328** (+51; `approvals`/`approval-aging`/`decision-outcome`/`edit-validate`/`approval-list`/`run-state-approvals` pure, `decision-dialog`/`approvals-page` render, `proxy` colon case); lint/typecheck/build/build:verify green; Go `TestApprovalListFixtureGolden` + api unit suite green, `make generate` diff-clean (no schema change). Playwright `dashboard-approvals.spec.ts` (4 — the three DoD + the live inbox listing). ADR-018 §"Approval inbox & decision UI (as built, 18.5)"; ADR-019 §"Approval inbox & decision UI (as built, 18.5)"; `web/app/README.md`. ROADMAP 18.5 ✅. **Next: 18.6** (ops views: DLQ, queue health, run controls).)*
...
**Done when:**
- [x] DLQ requeue round-trips from the UI and the run completes (e2e)
- [x] Controls hidden/disabled without the required scope (rendered-permissions test)
- [x] `/v1/system/stats` contract-tested; panel renders live under chaos load
