...
| `awaiting_human` | `succeeded` / `dead_lettered` / `cancelled` | decision recorded (ADR-017, ticket 15.3/15.4): single winner — human decision or timeout policy — under a CAS on the `approvals` row. Approve → the step succeeds with the (edited) payload; reject → dead-letter (`fail`) or route (`route`); a timeout `reject`/`approve` policy settles the same way but records the approvals row `expired` (source `timeout`); a timeout `park` policy leaves the step `awaiting_human` and parks the *run* (approvals `expired_at` marks it, migration 0027); run-cancel → cancelled | M15 |
