...
**Loop unrolling (as built, 14.3).** `dag.GenerateLoopExpansion` clones the loop
body (the `to`→`from` normal-edge span) into `<id>#k` instances, splicing the
next iteration after the completing loop-source instance and before-splicing the
loop source's exit edges onto the pending exit target. Two divergences from map:
(1) **body-only reference rewriting** — a loop body may reference pre-loop steps,
so only references to *body members* are suffixed (map bodies are self-contained,
so map rewrites every `steps.<x>`); (2) **constant iteration depth** — loop
iterations are sequential, not nested, so `ExpandRun` gained an
`ExpandRunArgs.DepthOverride` that pins every iteration to the loop source's
authored depth. Loops are bounded by `max_iterations`, **not** `MaxDepth` (which
exists to bound recursive planner nesting; applying it to a loop would cap
iterations at 4). A loop rejection is permanent (engine-generated), and
termination is governed by the loop edge's `on_exhausted` policy with a
`loop_exhausted` event (ADR-016 §"Loop-edge runtime").
