...
| `validation_failed` | **As built in M11** (ADR-013): the executor succeeded but output validation rejected the result. A *genuine* error class (unlike the administrative `lost`/`throttled`/`budget_exceeded`), recorded as the attempt outcome with the verdict persisted, but **excluded from the transport retry budget** and **rejected in `retry_on`** — it retries under its own semantic policy (`validation.max_attempts`, built in 11.4), not `retry`. The semantic budget is `CountValidationFailures` (validation_failed outcomes past the requeue baseline) — a separate query from the transport `CountCountedFailures`, so the two budgets are disjoint by construction and both surface per step in the status API (`transport_failures` / `validation_failures`). See the amended note below: ADR-013 gives validation its own policy rather than unlocking it in `retry_on`. | M11 |
...
| 20 | Context-assembly failure pre-execution (ticket 12.3, ADR-014: a required source resolved to nothing under `on_missing: error`, a `step_output` pointer did not resolve, a source's backend is unwired or names an unknown retriever, an executor that cannot inject context, or the materialized spec is corrupt) (`Engine.assembleContext`) | `permanent` | no retry; DLQ. Deterministic given store state — the same state assembles the same way, like row 15. A transport failure of a source read (a store/retriever hiccup) is row 11, not this row: nothing was decided, the delivery redelivers |
| 21 | Context over budget after compaction (ticket 12.4, ADR-014: the assembled request still exceeds `budget_tokens` after the compaction pipeline ran — the pinned set alone is too big, or the non-pinned set could not be shrunk enough — or a step declares a budget its executor cannot enforce because it counts no request) (`Engine.assembleContext` → `*contextmgr.OverBudgetError`) | `permanent` | no retry; DLQ, **before any provider call** (ADR-014's hard guarantee: overflow is never sent). Deterministic — the same assembly compacts the same way every attempt, like rows 15/20. The DLQ reason carries the budget/pinned/applied-strategies summary. *(12.5 note: a **summarizer** failure inside the pipeline — model unavailable, provider error, timeout, or a non-shrinking summary — is NOT this row. It never fails the step: the `summarize` strategy falls back to the next deterministic strategy and records a warning on its `context_revision`. This row fires only if the whole pipeline, fallbacks included, still cannot fit. A summarizer's caller-context cancellation is row 3/8, not this row — it redelivers. Any summarizer overhead that billed before this failure is still ledgered.)* |
| 22 | Planner plan rejected on **plan-attributable** grounds (ticket 13.1, ADR-015: bad `PlanOutput` shape, an invalid/duplicate/colliding injected step id, an unresolved endpoint, an illegal splice/anchor, a cycle the plan would introduce, a per-step config error, or exceeding the **per-expansion** `max_added_steps`) (`Engine`'s planner completion → `dag.ValidateExpansion`, `!CapExceeded`) | `validation_failed` | **not a transport failure**: the planner attempt records `validation_failed` (like any llm-family step, ADR-013) with the rejection issues as feedback, and M11's semantic-retry loop (11.4) re-prompts the planner. A *better plan can fix it*, so it retries under the semantic budget, not `retry`. The completion transaction rolls back — a rejected plan expands nothing |
| 23 | Planner plan rejected on **run-guard** grounds — an exhausted cap that no plan can satisfy (ticket 13.1, ADR-015: `max_total_steps`, `max_expansions`, `max_depth`, or the caller-resolved effective per-expansion cap when the run itself is out of budget) (`dag.ValidateExpansion`, `CapExceeded`) | `permanent` | no retry; DLQ (source `permanent`) with an `expansion_cap_exceeded: …` message, then the run's `on_failure` disposition. Deterministic given the run's size/count/depth and the caps — a better plan cannot lift the cap, so re-execution is provably futile, like rows 4–7/15/17/19. *(14.4 may add a `park`-and-raise-caps policy, mirroring budget parking; 13.1 fixes permanent-fail as the default.)* |
| 24 | Human-approval step **rejected** under `on_reject: fail` (ticket 15.3, ADR-017: an approver — or the `on_timeout: reject` policy — recorded a reject on a step whose reject routing is `fail`, the default) | `permanent` | **not a content failure of the executor**: no retry, no attempt-budget consumption; DLQ (source `permanent`) with an `approval_rejected: <comment>` message, then the run's `on_failure` disposition. A DLQ requeue re-runs the gate, producing a fresh pending approval. Under `on_reject: route` a reject is **not** a failure at all — the step *succeeds* and fires its `decision: reject` edges (ordinary routing, not this row). Reserved-for-15.3. |
...
expansion, ADR-015) inherit the same materialization path: `ExpandRun`
materializes each injected step's effective retry/timeout/validation/…
policy onto its `run_steps` row at expansion time, exactly as
instantiation does.
