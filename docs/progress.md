...
Added the run header's live cost meter, budget banners, and the raise-budget dialog. **Entirely under `web/app`** — no Go change, no migration, no config var, no metric. Every fact the meter needs was already on the event feed (10.5 wrote `cost_updated`'s running totals + budget for exactly this) and the raise/resume actions are the existing `PATCH /v1/runs/{id}/budget` + `POST /v1/runs/{id}/unpark` (10.3/5.6).

**The meter is stateless off the event feed.** The running totals ride `cost_updated` (`run_spent_nano_usd`/`run_saved_nano_usd`, non-decreasing in seq by construction — 10.5) and the budget rides `cost_updated`/`run_budget_updated` (the latter carries the new total on its payload). The 18.1 run-state reducer folds those into `run.cost` under the same `asOf` seq guard the rest of derived state uses, so the header updates live with no cost refetch.

**(1) `run-state.ts` additions.** `cost_updated` now also carries the run budget when the payload includes it; `run_budget_updated` sets the budget from its payload — **correcting the 18.1 placeholder comment** that claimed the budget total "isn't on the payload" (it is). `mergeRunResponse` (18.3) now also folds the body's run-level projection (`cost`/`status`/`park_reason`, via `{...state.run, ...body.run}`) when the body is at least as fresh as `asOf`, so a refetch after a raise/unpark reconciles those fields but a stale body never regresses the event-derived live status/totals.

**(2) Pure derivations** (`src/lib/pure/dashboard/`, the no-React boundary, unit-tested):
- `cost-meter.ts` — `deriveCostMeter(run)` projects the folded `CostSummaryView` onto `{spent, saved, budget?, fraction?, tier, parkedForBudget}` with a `BudgetTier` (`unbudgeted`/`ok`/`warn`/`danger`/`exceeded` at 75/90/100%; `exceeded` also when the run is parked with `park_reason==="budget_exceeded"`); `foldCostEvents` folds a run of events onto `{spent, saved, budget}` and a test proves it converges to the Go cost golden's `summary` (the DoD-3 consistency assertion, pre-paid at the reducer level); `formatUsd`/`nanoToUsd` helpers.
- `budget-banners.ts` — `deriveBudgetBanners(events, run)` projects `model_downgraded`/`budget_exceeded`/`run_budget_updated` onto typed, **seq-keyed** banners (dedupe across a reconnect's re-backfill; downgrade banners carry `from`/`to`/`trigger` + a resolved trigger label — "budget threshold 50%" / "budget projection (run)" — for the DoD-2 assertion), newest-first, and pins a **live "parked at budget cap" affordance** (synthetic key `-1`) while `status==="parked" && park_reason==="budget_exceeded"` (gone on unpark) that carries the Raise action.

**(3) Actions** (`useBudgetActions.ts` + `streams.ts`). `setRunBudget(runId, usd)` (`PATCH .../budget`) and `unparkRun(runId)` (`POST .../unpark`) through the proxy, classifying the error envelope (`invalid_request` → 400, `conflict` → 409, `forbidden` → 403). `useBudgetActions.raiseBudget({budgetUsd, resume})` does the documented resume path (ADR-012): raise, then — if the user opted in — unpark. The park→resume is reflected live through `run_budget_updated` + `run_unparked`, so the hook holds **no optimistic state** — it tracks pending/error and asks the controller to `refreshViews()` as belt-and-braces. A 409 on the *unpark* (the run already resumed via a concurrent action, or was not parked) is tolerated — the raise still succeeded.

**(4) Components.** `CostMeter` (header ticker with `data-tier`/`data-spent-nano`/`data-saved-nano`/`data-budget-nano`, a `role="progressbar"` budget bar with tier colouring, a "parked at cap" marker, and a saved-by-cache indicator), `BudgetBanners` (dismissible stack; the parked banner carries a Raise button; `data-testid="banner-<kind>"` + `data-from`/`data-to`/`data-trigger`), `RaiseBudgetDialog` (prefilled USD input — `max(2× budget, 2× spent)`; warns-not-blocks when the entry is below current spend; a resume checkbox defaulting on when parked for budget). They replace the 18.1 ad-hoc `$` span in `RunDetail`, which now also shows a "Raise budget" affordance whenever the run is budgeted or parked.

**Decisions.** The meter is stateless off `cost_updated` (the stream totals are authoritative and monotonic — no client accumulation, which would risk drift); raise + optional unpark in **one dialog** (the ticket's "park→resume reflected live"); warn-not-block on a sub-spend budget (the operator's call); no backend change — every fact was already on the feed and the REST budget PATCH. **Accepted residual:** the e2e budget arithmetic is calibrated to the offline mock pricing (`mock:*` = $1/$2 per Mtok) with **response caching disabled per step** (`cache: {mode:"off"}` — the global-scoped response cache would otherwise serve a repeat run for $0 and the run would never climb or park).

**Tests.** App vitest **248 → 277** (+29): `cost-meter.test.ts` (tiers/formatting/`foldCostEvents` convergence to the Go cost golden), `budget-banners.test.ts` (derivation, dedupe, park-clears-on-unpark, trigger labels), `cost-meter.test.tsx`/`budget-banners.test.tsx` (render), `raise-budget-dialog.test.tsx` (mocked `streams`: PATCH-then-unpark order, resume-unchecked ⇒ no unpark, 409-on-raise surfaced + stays open, 409-on-unpark tolerated, sub-spend warning), and `run-state`/`run-state-merge` extensions (budgeted `cost_updated`, `run_budget_updated`, budget park/unpark status, the run-level merge fold + stale-body guard). Lint/typecheck/build green. Playwright `dashboard-cost.spec.ts` (against compose): DoD-1 (a tight-budget mock chain climbs `data-spent-nano`, parks with the parked banner + `data-tier="exceeded"`, Raise → $1 + resume → `succeeded`, all live, + DoD-3 meter-total == `GET .../cost` summary), DoD-2 (a soft-threshold `model_fallbacks` run → `banner-downgrade` naming `mock/sim-1 → mock/cheap`, trigger `budget_threshold`).

**Docs.** ADR-018 §"Live cost meter & budget UX (as built, 18.4)"; ADR-019 §"Live cost meter & budget UX (as built, 18.4)"; `web/app/README.md`. ROADMAP 18.4 ✅. **Next: 18.5** (HITL approval inbox & decision UI).

**Local-setup note.** 18.4 has **no migration and no backend change**, so the existing compose stack (already at migration 0028 from 18.3) runs the new e2e as-is — no rebuild needed unless the stack predates 18.3. Run `web/app`'s Playwright specs against a running app-profile compose stack with `AGENTLOOM_API_URL`/`AGENTLOOM_API_KEY` set (see `scripts/web-e2e-smoke.sh`).

### 18.5 — HITL approval inbox & decision UI ✅

**What it delivered.** The human-approval inbox (`/approvals`) and the decision UI (a shared decision dialog, a "waiting on you" run-header banner, a Decide affordance on the gate's DAG node, and an InspectorPane Approval tab), built on the 15.3/15.4 approval API and the 18.1–18.3 dashboard plumbing. **Entirely under `web/app` plus two additive, non-runtime backend touches** — no migration, no config var, no metric, no OpenAPI change. The six `approval_*` events and the two REST routes (`GET /v1/approvals`, `POST /v1/approvals/{id}:decide`) were already complete from M15, and the generated api-client types already carried the operations/schemas (no regeneration).

**The two backend touches.** (a) **A proxy fix** (`web/app/src/app/api/agentloom/[...path]/route.ts`): the same-origin proxy joined path segments with `encodeURIComponent`, turning `approvals/<id>:decide` into `…%3Adecide` — which the chi router (matching on the raw path with `:` as its param delimiter) 404s before reaching `handleDecideApproval`. The proxy now preserves a literal `:` verb suffix (`encodeSegment` encodes each colon-delimited part independently), pinned by a new `proxy.test.ts` case asserting the upstream URL keeps `:decide`. This was the one real blocker the survey surfaced — the decide call would have silently 404'd. (b) **An exported Go golden** `internal/api/testdata/approval_list_fixture.json` (`TestApprovalListFixtureGolden`, `UPDATE_GOLDEN=1`, the 13.6/18.3 precedent) — the exact `GET /v1/approvals` wire shape covering pending (with edit_schema + timeout) / approved-with-edit / rejected / expired / **park-expired** (`on_timeout: park` — `expired_at` stamped but status still `pending`, still decidable) rows, which the frontend approval tests read via `fs` as ground truth.

**(1) api-client.** Four ergonomic re-exports from `web/lib/api-client/src/index.ts` (`ApprovalView`/`ApprovalListResponse`/`DecideApprovalRequest`/`DecideApprovalResponse`) — the generated types already existed; the index just didn't surface them.

**(2) Pure layer** (`src/lib/pure/dashboard/`, no-React eslint boundary, vitest-tested against the golden): `approvals.ts` (the core reducer — `ApprovalRecord` = the view + a per-record `lastSeq` cursor + a `partial` flag; `applyApprovalEvent` folds the four `approval_*` events absolutely by status under the seq guard: an `approval_requested` seeds a *partial* record since the event carries no payload/description/edit_schema, `approval_expired` with `action: run_parked` stays `pending` + stamps `expired_at`; `mergeApprovalViews` completes a partial from the authoritative run body without regressing a fresher live record; `decidable`/`parkExpired`/`pendingForStep`/`recordsForStep` selectors). `approval-aging.ts` (`deriveAging` fresh/aging/stale at 1h/24h, `deriveDeadline` countdown/overdue, `humanizeDuration`; `now` injected). `decision-outcome.ts` (`rejectPlan` fail-vs-route + targets from the gate's `on_reject` config and the topology's reject-marked edges; `approveTargets`; `decisionOutcome` rendered text for a decided/expired record). `edit-validate.ts` (a small CSP-safe JSON-Schema *pre-check* mirroring JSON-Schema **validator** semantics — enforces `required`/`enum`/`type`, allows extra props unless `additionalProperties:false` — deliberately NOT the strict Go decoder / graphdef `checkShape` (which forbids extra props and skips required/enum), so it never over-rejects; issue paths are RFC-6901 pointers matching the backend). `approval-list.ts` (the inbox row model + `applyInboxEvent` + status/run filter codec, oldest-first insertion). `run-state.ts` gained `RunState.approvals` folded from both the snapshot body (`approvalsFromViews`) and the live feed (`applyApprovalEvent`); `projection.ts` carries a `pendingApprovalId` on a gate node (from `pendingForStep`) so the DAG node can render a Decide affordance without the pure layer importing React.

**(3) Glue** (`src/lib/dashboard/`): `streams.ts` `listApprovals`/`decideApproval` (a typed `DecisionOutcome` distinguishing 409 `approval_not_pending`, 422 `approval_decision_invalid`+issues, and the ok response) + `refetchApproval` (there is no `GET /v1/approvals/{id}`, so it re-lists the run's page and finds the row); `useApprovalActions` (drives the decide, classifies the outcome, holds no optimistic state); `useApprovalInbox` (the `useRunListLive` mirror — a firehose subscription over the approval events, run-scoped `run_ids` filter, discovery of an unseen gate by re-listing its run on `approval_requested`); `RunController.onEvent` refetches the run body on a live `approval_requested` (coalesced `refreshViews`) so a partial record fills in with the payload/edit_schema before the dialog opens. `useRunGraph`/`RunGraph` thread the approvals map + an `onDecide` callback (injected into node data like `RunGroupNode.onToggle`).

**(4) Components.** `DecisionDialog` (the shared modal — a `JsonViewer` over the proposed payload, an optional schema-validated JSON editor gated on `allow_edit`, a comment, approve/reject with the reject-plan hint, client-side `edit-validate` issues + server 422 issues, and the **409-recovery read-only "decided in another session" state** — reached from the 409 on submit or the live `approval_decided` event, whichever wins); the `/approvals` inbox page + table (aging chip, deadline countdown, editable badge, decided-outcome subtext, status+run filters URL-synced, keyset "Load more", live connection pill); `ApprovalBanner` (the run-header "N waiting on you" affordance beside the 18.4 budget banners); a `RunStepNode` footer Decide button (stopPropagation so it doesn't select the node); an InspectorPane **Approval** tab (`ApprovalTab` — the record, aging, outcome, and a Decide button while pending). Plus a `destructive` Button variant (the reject button).

**Decisions.** Apply the decide *response* immediately (it already carries the decided approval; the event feed confirms) rather than an optimistic pre-response guess — no rollback logic. Edit validation is a client pre-check; the server 422 is authoritative. Inbox discovery via `?run_id=` (no single-approval GET exists; not worth adding). The reject-outcome hint derived from `StepView.config` (18.3) + graph-edge `decision` markers (18.2) — zero backend change. Scope-gated hiding of the Decide control is 18.6's rendered-permissions concern; 18.5 surfaces a 403 inline.

**Accepted residuals.** No nav-wide pending badge (the inbox page counts only — an `engine_approval_pending` gauge exists for a future badge); a per-field structured edit form stays deferred (ADR-017 lists it) — the dialog uses a raw JSON editor with pointer-keyed issues; the firehose's `MaxTrackedRuns` bound applies to the live inbox (a REST refetch heals). The Playwright e2e drives the identical UI path on the deterministic `approval_gate.json` shape offline rather than the flagship (whose gate is decided in the Go `TestFlagshipResearchCriticWriter`).

**Tests.** App vitest **277 → 328** (+51): `approvals`/`approval-aging`/`decision-outcome`/`edit-validate`/`approval-list`/`run-state-approvals` pure suites (folded against the golden), `decision-dialog`/`approvals-page` render suites, the `proxy` colon case. Lint/typecheck/build/`build:verify` green; api-client + graphdef + engine-client suites unaffected. Go `TestApprovalListFixtureGolden` + the api unit suite green, `go build ./...` clean, `make generate` diff-clean (no schema change — only the hand-edited api-client index moved). Playwright `dashboard-approvals.spec.ts` (4 specs — DoD-1 approve-with-edit → downstream payload, DoD-2 concurrent-decision 409 recovery, DoD-3 reject routing + outcome, and the live inbox listing).

**Docs.** ADR-018 §"Approval inbox & decision UI (as built, 18.5)"; ADR-019 §"Approval inbox & decision UI (as built, 18.5)"; `web/app/README.md` (dashboard intro widened to 18.1–18.5, a new §"The approval inbox & decision UI (18.5)", the `/approvals` route bullet, the proxy colon note). ROADMAP 18.5 ✅. **Next: 18.6** (ops views: DLQ, queue health, run controls).

**Local-setup note.** 18.5 has **no migration and no backend runtime change**, so the existing app-profile compose stack (at migration 0028 from 18.3) runs the new Playwright e2e as-is — no rebuild needed (the proxy fix and the new pages are all in the `web/app` build, which `scripts/web-e2e-smoke.sh` rebuilds). The e2e is fully offline: the gate definitions run on the echo mock (`AGENTLOOM_LLM_MOCK_ENABLED=true`, no mock script needed), and the root key carries the `approve` scope. To run: an app-profile compose stack up (`docker compose --profile app up -d --wait`), then `web/app`'s Playwright with `AGENTLOOM_API_URL`/`AGENTLOOM_API_KEY`/`AGENTLOOM_API_PUBLIC_URL` set — or `make web-e2e`.

### 18.6 — Ops views: DLQ, queue health, run controls ✅ (M18.6)

**What it delivered.** The operator surface for M18: a DLQ page (cross-run dead-lettered steps with error/attempt context + requeue), run controls (cancel/park/unpark) with scope-gated buttons, and a queue-health mini-panel — backed by three additive read-only endpoints. **Under `web/app` plus a focused backend addition — one migration (0029, two additive indexes), three new `read`-scoped endpoints, no config var, no metric, no new event, no new outcome/class.**

**The three endpoints (ADR-018).** (1) `GET /v1/system/stats` — a point-in-time queue-health snapshot: ready depth / PEL / delayed / worker roster (from Redis Streams introspection via a new **read-only** `api.QueueIntrospector` seam), plus the DLQ backlog / outbox backlog / active-run count (from Postgres). Postgres is the only hard dependency, so the endpoint **always answers 200**: when queue introspection is unwired (no Redis on this API) or a queue read fails, `queue` is `null` + `queue_error` and the Postgres figures still render. A read is not a dispatch — ADR-002 holds. (2) `GET /v1/dead-letters` — a cross-run keyset page of dead-letter records, newest-first, each joined to its **live** step/run status so the operator sees what is still `open` (a step still `dead_lettered` at its latest death); `status` (open|all), `run_id`, `source` filters; the requeue action itself is the existing `POST …/steps/{sid}/requeue` (6.5). (3) `GET /v1/auth/whoami` — the caller's own key id + scopes, which is **what makes rendered permissions possible at all** (the browser talks through a server-held key and otherwise has no way to learn its scopes). All three are `read`/`read` in the route tables; the whole set closes 18.5's deferred "scope-gated control hiding is 18.6's".

**Backend.** Migration 0029: `dead_letters_created_idx (created_at DESC, run_id DESC, step_id DESC, seq DESC)` for the uniform-descending keyset, and a partial `run_steps_dead_lettered_idx … WHERE status='dead_lettered'` for the open-count/open-join. Queries `ListDeadLettersPage` (joins + filters + `open` derivation via `seq = MAX(seq) AND status='dead_lettered'` + keyset), `CountOpenDeadLetters`, `CountActiveRuns`; repo methods `DeadLetterRepo.{ListPage,CountOpen}` (+ `DeadLetterPageArgs`/`DeadLetterCursor`) and `RunRepo.CountActive`. `queue.ListConsumers` (XINFO CONSUMERS → name/idle/pending, sorted). The `QueueIntrospector` seam (a narrow `QueueStats(ctx) (QueueStatsView, error)` interface + `WithQueueIntrospector` option) is satisfied by a thin cmd/api adapter over the `*queue.Queue`+`*queue.Delayed` cmd/api already builds (15.4), so `internal/api` never imports `internal/queue`/go-redis (the CacheOps discipline); a not-yet-bootstrapped group (`ErrNoGroup`) is reported as an empty-but-present queue (zeros), not an error. Handlers `system.go` (whoami + system stats) and `deadletters.go` (list + opaque cursor codec + view builder). `ctl ops {whoami,stats,dlq}`. OpenAPI: three ops (`getSystemStats`/`listDeadLetters`/`whoami`) under a new `System` tag + schemas (100/100 lint; route-coverage/operation-contract tests green); api-client regenerated. Exported Go goldens `internal/api/testdata/{dead_letter_list,system_stats}_fixture.json` (`TestDeadLetterListFixtureGolden`/`TestSystemStatsFixtureGolden`, `UPDATE_GOLDEN=1`) the frontend tests read as ground truth.

**Frontend.** api-client re-exports (`DeadLetterView`/`DeadLetterListItem`/`DeadLetterListResponse`/`SystemStatsResponse`/`QueueStatsView`/`ConsumerView`/`WhoAmIResponse`/`Cancel|Park|Unpark|RequeueStep*Response`/`Scope`/`StepStatus`). **Pure layer** (`src/lib/pure/dashboard/`, no-React boundary): `permissions.ts` (`hasScope` with admin-implies-all, `can`, control→scope map, `controlState` → hidden|disabled|enabled: **disabled while loading, hidden when a scope is definitely missing, enabled when whoami is unavailable** — fail-open for UX, the server stays the authority), `run-controls.ts` (`runControls(status)` availability + `requeueable`), `dead-letters.ts` (`DlqRow` + `applyDlqEvent` under a seq guard + filter/URL codec + `upsertDlqRow` newest-first, folded against the Go golden), `system-stats.ts` (`deriveStatTiles` health tiers + `deriveStaleness` with injected `now`). `run-state.ts` gained `RunState.deadLetters` (step-keyed, folded from the run body). **Glue** (`src/lib/dashboard/`): `streams.ts` `cancelRun`/`parkRun`/`requeueStep`/`listDeadLetters`/`fetchSystemStats`/`fetchWhoAmI` (+ the exported `classifyActionError`/`RunActionOutcome`), `useRunControls` (drives the op, holds no optimistic state, refreshes the body — which also pulls `cancel_reason`, absent from the event payload — and tolerates a 409 as a soft outcome), `useSystemStats` (visibility-aware polling), `useDeadLetterList` (the `useApprovalInbox` mirror — firehose over `step_dead_lettered`/`step_requeued`, refetch-by-run for a fresh death's error context); `src/lib/permissions.tsx` (`PermissionsProvider` fetching whoami once, mounted in the root layout; `usePermissions` safe outside a provider → fail-open). `RunController` now also refetches the body on a live `step_dead_lettered` (the event carries no error document). **Components:** `RunControls` (header Park/Unpark/Cancel group, scope-gated, Cancel confirms), `QueueHealthPanel` (tiles + worker roster + "as of" staleness), the `/ops` page (queue-health panel + DLQ table with expandable error JSON, URL-synced filters, keyset paging, live pill, requeue), an InspectorPane Requeue affordance for a dead-lettered step, and a nav "Ops" link. The 18.5 Decide affordances gained `approve`-scope gating (`ApprovalBanner` `canDecide`), and the Raise-budget button `submit`-scope gating.

**Decisions.** A real cross-run DLQ **list endpoint** rather than fanning out over `GET /v1/runs?status=failed` (the `step_dead_lettered` event carries no error and the run body is per-run — a list is the honest minimum, the `GET /v1/approvals` precedent). whoami under `read` scope (a key without `read` cannot render the dashboard anyway). The stats endpoint degrades to `queue: null` rather than 503 (Postgres is the only hard dependency — the DLQ/runs figures stay useful). Permissions fail open (loading → disabled, definite-missing → hidden, unavailable → enabled) — the server enforces every scope, so the browser view is a UX affordance. One `/ops` page hosting both the panel and the DLQ table.

**Accepted residuals.** The InspectorPane Requeue and the `/ops` DLQ requeue are two entry points (both hit the same 6.5 route). The node-footer/inspector Decide buttons are not scope-gated (only the prominent ApprovalBanner Decide is) — they lead to the DecisionDialog which surfaces a 403 inline (18.5). The queue-health panel polls (no event carries queue depth).

**Tests.** App vitest **328 → 371** (+43): `permissions`/`run-controls`/`dead-letters`/`system-stats`/`run-state-deadletters` pure (folded against the Go goldens), `run-controls` render (the DoD-2 rendered-permissions matrix: read-only → hidden, loading → disabled, submit → enabled, whoami-unavailable → enabled), `ops-page` render. Lint/typecheck/build/`build:verify` green; api-client/graphdef/engine-client suites unaffected. Go: `TestDeadLetterListFixtureGolden`/`TestSystemStatsFixtureGolden` + `TestListConsumers` (queue) + `TestListDeadLettersAcrossRuns`/`TestSystemStats`/`TestWhoAmI` (api integration, over the newFleet harness with the introspector wired) green under `-race`; golangci-lint clean (incl. integration tag); `make openapi-lint` 100/100; `make generate` diff-clean; migrate round-trip → 29. Playwright `dashboard-ops.spec.ts` (3 — DoD-1 requeue-from-`/ops`→run-succeeds→row-closes; run controls park→unpark→cancel; queue-health panel renders live figures) verified green against compose. **A collateral 18.5-test fix:** `dashboard-approvals.spec.ts` DoD-2 (concurrent-decision 409) was failing deterministically on a fresh compose stack — root-caused to a timing-fragile *test* (not the feature): on a fast stack the live `approval_decided` event arrives before the test's `decision-approve` click, swapping the dialog to the conflict view and removing the button, so the unbounded `.click()` auto-waited the full timeout and burned the test budget before the conflict assertion ran. Verified failing **identically on the pristine HEAD UI** (unrelated to 18.6) and unaffected by tearing the compose volumes down to zero backlog (not load-related). Fixed by bounding the best-effort click (`{ timeout: 3000 }`) so a vanished button can't hang the test; all four `dashboard-approvals` specs now pass 3×, and the approvals+ops set (7) passes together on a clean stack.

**Docs.** ADR-018 §"Ops views (as built, 18.6)"; ADR-019 §"Ops views (as built, 18.6)"; `web/app/README.md` (dashboard intro widened to 18.1–18.6, a new §"Ops views (18.6)", the `/ops` route bullet); `docs/api.md` (the three endpoints). ROADMAP 18.6 ✅. **Next: M19** (load testing & bottleneck remediation).

**Local-setup note.** 18.6 **adds migration 0029 and new backend endpoints**, so the compose app stack must be migrated + rebuilt before the new Playwright e2e (and the live endpoints) work: `make migrate-up` (applies 0029 — two additive indexes) then `docker compose --profile app up -d --build --wait` (rebuilds the api/worker binaries with the new endpoints). The Playwright e2e is offline (the DLQ definition runs on the echo mock via `fail_n_times`; the root key carries every scope). Run: `web/app`'s Playwright with `AGENTLOOM_API_URL`/`AGENTLOOM_API_KEY`/`AGENTLOOM_API_PUBLIC_URL` set — or `make web-e2e`.

## Milestone 19 — Load testing & bottleneck remediation (local)

### 19.1 — Load test plan & pinned environment ✅ (M19.1)

**What it delivered.** The reviewed load-test plan, a one-command resource-pinned load environment, and the CI-validated scenario/definition corpus — the substrate the load generator (19.2) and baseline campaign (19.3) run on. Docs-and-environment ticket with three small, additive code changes: the mock provider's latency/token simulation, pprof on the admin listener, and a `internal/loadtest` scenario-contract leaf package. **No migration, no new metric, no schema change; the one config addition is `AGENTLOOM_OBS_PPROF_ENABLED`.**

**The plan (`docs/load/plan.md`).** SLO targets (sustain ≥1,000 concurrently active runs for 10 min; scheduling latency ready→running p50 ≤ 250 ms / p99 ≤ 2 s; API submit p99 ≤ 100 ms; zero lost runs / duplicate side effects / non-deliberate dead letters), precise definitions (concurrently-active-run, scheduling latency as the primary saturation signal measured at the claim CAS via `engine_step_scheduling_latency_seconds`), the five scenarios, open-loop measurement methodology (warmup/steady-state/cooldown windows, percentile sourcing from Prometheus histograms + loadgen HDR, per-resource saturation evidence with `pg_stat_statements`/pprof/Redis INFO capture recipes), the integrity assertions asserted at load (exactly-once step execution, gap-free `events.seq`, quiescence, no unexpected DLQ), and — the honesty requirement — **seven pre-registered bottleneck hypotheses ranked by prior likelihood**, each with its confirming signal and pre-authorized remediation: H1 serial per-worker execution (the likely first binder — one in-flight step per worker process, so fleet step-concurrency = replica count), H2 Postgres write amplification per completion, H3 outbox drain contention, H4 join-counter hot row, H5 single-stream serialization/Redis CPU (→ the ADR-005 stream-sharding lever, 19.5), H6 submission-path instantiation cost, H7 pgx pool exhaustion.

**The pinned environment (`docker-compose.load.yml` + `deploy/load/postgres-init.sql`).** A compose **overlay** on top of the base stack (not a copy — the base file stays the single source of truth), booted by `make load-up` (→ `scripts/load-env.sh`, which layers the two files, echoes the resource pins, and prints service status). Postgres pinned 2 CPU / 2 GiB with `pg_stat_statements` preloaded (the init SQL creates the extension on a fresh volume) and tuned `shared_buffers`/`work_mem`/`max_connections`; Redis 1 CPU / 768 MiB; api 1 CPU / 768 MiB; worker × `AGENTLOOM_LOAD_WORKERS` (default 8) at 0.5 CPU / 384 MiB each. **Deliberate choices, all documented in the plan:** response cache OFF (the global-scoped M9 cache would serve an identical-prompt load run for $0 with no provider call — it would measure the cache, not the engine); API rate limits raised ~200× so they never bind but the 429 path stays wired; OTel export off by default (`AGENTLOOM_LOAD_OTEL` opt-in) since sampling overhead confounds CPU profiles; steplog left at `info` (it is itself an H2 input — measure reality); **dedicated load volumes** (`postgres-load-data`/`redis-load-data`) so campaigns start pristine and never touch dev/CI data (`make load-nuke` drops them). Prometheus already discovers the scaled workers via `dns_sd_configs`, so no scrape change.

**The scenario corpus (`test/load/` + `internal/loadtest`).** Four offline workflow fixtures under `test/load/definitions/` — `linear_10` (10-step chain, the cleanest H1 probe), `fanout_50` (seed → 50 parallel → `join(all)` → final; 53 steps, one join-counter hot row, a 53-row instantiation tx), `planner_heavy` (two sequential `planner` runtime expansions, each 8 workers → join; the planner prompt IS a valid `PlanOutput` echoed verbatim by the mock so it expands offline), `agent_loop` (writer⇄critic always-revise → exactly 3 loop-backs → `on_exhausted: proceed`) — plus five named scenario configs under `test/load/scenarios/` (`linear-10`/`fanout-50`/`planner-heavy`/`agent-loop`/`mixed`) carrying arrival profile (constant + ramp-to-knee), warmup/duration windows, and SLO targets. The new leaf package `internal/loadtest` (imports only `internal/dag` + stdlib) is the scenario contract: `Scenario`/`Arrival`/`SLO`/`MixEntry` types, a strict `Parse` (unknown-field-rejecting, shape-validating), and `LoadDir` which cross-checks that each referenced definition file decodes + passes full `dag.Validate` (the same gate submission applies) and that every mix entry resolves. `TestScenarioCorpusLoads` walks the committed corpus — so **"scenario definitions runnable as named configs" is a CI unit test, not a promise**, and the load generator (19.2) reuses the same parser. `test/load/mock.json` is the fleet mock script (lognormal p50 120 ms / p99 900 ms, uniform token draws, the always-revise critic rule) mounted into every worker via `AGENTLOOM_LLM_MOCK_SCRIPT_FILE`.

**Mock provider load distributions (`internal/llm`, additive).** To give the compose fleet realistic simulation, `LatencySpec` gained a **lognormal** mode (`P50`/`P99` → μ/σ, drawn via `rng.NormFloat64`) alongside the existing fixed/uniform modes, and a new `TokenDist` (`{Input, Output TokenSpec}`) lets an outcome draw its reported `Usage` from a distribution instead of the char-count estimator (an explicit `Usage` still wins; the draw happens under the same lock as the latency draw, keeping the seeded transcript deterministic). Both default nil/zero, so the 12.1 mock token-counter parity holds unless a script opts in. The offline-run wire contract `ParseMockScript` gained matching `latency`/`tokens`/`inject` blocks (global) and per-outcome `latency`/`tokens`/`usage` overrides, durations as Go-duration strings, unknown fields rejected — the same strict-decode leaf-parser discipline as the 14.5 script.

**pprof on the admin listener.** `metrics.Listen` gained a `WithPprof` option mounting `net/http/pprof` (`GET /debug/pprof/...`) on the same in-network admin port that serves `/metrics`, gated by the new `AGENTLOOM_OBS_PPROF_ENABLED` (off by default; the load overlay turns it on for both deployables). Off by default the handlers are absent; when enabled the listener's write timeout is dropped (a CPU profile takes 30–60 s), acceptable on a debugging-only in-network port never published to the host. This is what 19.3's worker/API profiling captures against.

**Decisions.** Overlay (not a standalone compose) with dedicated volumes; cache disabled at the env level; OTel off by default under load; the integrity dup-effect probe **deferred to 19.6** (the four baseline fixtures are pure mock-`llm` work to stay offline and volume-free; the `effectful_echo`/`counter` journal probe needs a shared writable volume + per-run unique paths, wired in 19.6 — documented in the plan's §6); lognormal latency via `p50`/`p99`, token spec uniform, injection rates carried on the wire but 0 in the default script; pprof wired now (19.3 depends on it) rather than deferred. **Accepted residual:** the integrity workload is 19.6's; the four scenarios' at-load exactly-once assertions rest on the existing quiescence + event-seq checks until then.

**Tests.** `internal/llm` mock unit suite extended (lognormal latency draw distribution, token distribution + explicit-usage precedence, construction-validation for the new failure modes) + `mockscript` parse matrix (distribution wire fields, bad-duration rejection); `internal/loadtest` (corpus loads with every definition validated, arrival-shape parse matrix, shape-rejection matrix, LoadDir missing-definition + unknown-mix-ref rejection); `internal/obs/metrics` pprof-mounted-when-enabled + absent-by-default; `internal/config` obs-config PprofEnabled round-trip. All unit suites green; `golangci-lint` clean; `docker compose -f docker-compose.yml -f docker-compose.load.yml config` validates the merged overlay.

**Docs.** `docs/load/plan.md` (new); `test/load/README.md` (new); `docs/README.md` index (load plan promoted from "planned"); ADR-009 §"Mock/simulation provider" gained a "Load distributions (added 19.1)" note; ADR-008 §"Wiring" gained a pprof-on-admin-listener note; `.env.example` (`AGENTLOOM_OBS_PPROF_ENABLED`, `AGENTLOOM_LOAD_WORKERS`, `AGENTLOOM_LOAD_OTEL`). ROADMAP 19.1 ✅. **Next: 19.2** (load generator — `cmd/loadgen`, open-loop arrival control, run-lifecycle tracking, HDR histograms, JSON/CSV report artifact).

**Local-setup note.** Nothing to migrate (no schema change). To exercise the load environment: `make load-up` builds the images and boots the pinned overlay (app + obs profiles, 8 workers) with the load mock script mounted — this is heavier than `make up-app` because it builds the Go images and scales the fleet, so give the Docker Desktop VM ≥ 12 GiB before a full run. `make load-status` shows the fleet; `make load-down` stops it; `make load-nuke` drops the dedicated load volumes for a pristine restart. There is no load campaign to run yet — the generator that drives the scenarios is ticket 19.2.

### 19.2 — Load generator ✅ (M19.2)

**What it delivered.** `cmd/loadgen` + the `internal/loadgen` engine: an open-loop load generator that drives the 19.1 scenario corpus against a running API, tracks every run's full lifecycle (submit → terminal), and writes an HDR-percentile report artifact with a failure taxonomy and integrity assertions. **No migration, no new config var, no new metric, no backend change, no new dependency** — it talks to the engine only through the existing public HTTP API + firehose, and reuses `internal/loadtest`'s scenario parser so the generator and the CI-validated corpus share one contract.

**The package (`internal/loadgen`).** A leaf over `internal/loadtest` + `internal/api` (wire types) + `internal/event` (the firehose envelope) + coder/websocket + stdlib. Split into pure and IO halves so the arrival math and the lifecycle machine are unit-testable without a server:
- **`schedule.go` (pure).** `buildSchedule` yields the deterministic open-loop fire sequence: inter-arrival gaps are the reciprocal of the instantaneous rate at the current offset (constant, or a piecewise-constant ramp staircase clamped at `to_per_sec`), bounded by the window and/or `--runs`. `mixDraw` is the seeded weighted picker for a composite (`mixed`) scenario. The intended schedule is a pure function of the rate — nothing downstream shifts it.
- **`pacer.go` (clock-injectable).** `runPacer` walks the schedule, sleeping until each fire's intended time (`start + offset`) then calling a **non-blocking** dispatch (production spawns a submit goroutine), so a saturated system never shifts a later fire's intended time — the coordinated-omission guard, proven by a fake-clock test that jumps wall-time 5s ahead inside a dispatch and asserts every subsequent fire still targets `start + offset`.
- **`hist.go` (pure).** A hand-rolled **log-linear HDR histogram** (one bucket per multiplicative growth step, 1% relative error, bounded memory, exact min/max/mean via a running sum) — hand-rolled rather than a dependency so the module stays buildable offline (the tiktoken/embed precedent). `Snapshot()` is the reported p50/p90/p95/p99/p999 set; `distribution()` is the per-histogram CSV.
- **`tracker.go`.** The lifecycle state machine under one mutex: `registerFire` (records an intended submission before it fires, so an inflight-cap skip or a submit error is still tracked — nothing is silently omitted), `recordSubmit` (files the outcome into the taxonomy + the submit-RTT and submit-from-intended histograms), `applyEvent` (folds a firehose event: `step_ready`→`step_claimed` pairing into the scheduling histogram for sampled runs — a retry's claim has no ready pair and is excluded, matching 7.2; terminal events mark the run terminal once, first-writer-wins), `applyRunBody` (folds a polled/reconciled run body), and `finalizeOpen`/`markLost`. **Skew-corrected end-to-end latency**: server terminal timestamps are corrected into the client frame by the estimated skew, then differenced against the client submit time — free of both server↔client skew and firehose/poll delivery lag.
- **`client.go`.** A minimal API client with a transport tuned for many concurrent short requests (`MaxIdleConnsPerHost` raised so a 60+/s submit stream reuses connections). `registerDefinition` POSTs a definition (409 → append a new version), so **every campaign targets a fresh `definition_id`** and the reconciliation walk is exact.
- **`firehose.go`.** Tails `/v1/events/ws` with a `types` filter, authenticating with a **read bearer on the upgrade** (the non-browser path — no ticket dance), dedupes by `(run_id, seq)`, reconnects with backoff. Firehose terminal detection is a low-latency *sampling* channel; per-run polling is the authority, so a missed frame is never fatal.
- **`runner.go` / `runner_phases.go`.** The orchestration: setup (parse + register definitions + clock-skew/DLQ probe) → optional firehose → open-loop pacer (non-blocking dispatch, optional `--max-inflight` semaphore) → drain (wait for accepted runs to reach terminal, bounded by `--run-timeout`, poll-backed) → **reconcile** (`GET /v1/runs?definition_id=` per campaign definition, cross-check the submitted set → the "no lost runs" assertion) → **quiescence** (poll `/v1/system/stats` until ready/pending/delayed/outbox all zero, bounded by `--drain-timeout`) → assemble + write artifacts. A per-tick progress line + a per-tick timeseries sample run throughout.
- **`report.go` / `markdown.go`.** `summary.json` (config, windows, definition ids, **rate accuracy** [offered vs achieved over the actual steady arrival span + pacer lag], latency percentiles for submit-RTT / submit-from-intended / end-to-end / scheduling, throughput, active-run peak, **failure taxonomy**, integrity [lost runs, non-deliberate DLQ], quiescence, SLO evaluation, clock skew), `summary.md` (human tables), `runs.csv` (per-submission rows), `timeseries.csv` (per-tick queue depths), `hist-*.csv` (HDR distributions).

**Failure taxonomy (DoD-3).** Every submission lands in exactly one class summing to the total: `run_succeeded`/`run_failed`/`run_cancelled`, `run_timeout` (accepted, never terminal by deadline), `run_lost` (accepted but absent from reconciliation — must be 0; `--fail-on-lost` exits non-zero), `submit_http_4xx` (examples carry the API error code, e.g. `rate_limited`)/`submit_http_5xx`/`submit_transport_error`/`submit_timeout`, and `skipped_inflight_cap`.

**The CLI (`cmd/loadgen`).** stdlib `flag`, single command: `--scenario`/`--list-scenarios`, arrival overrides (`--rate`, `--ramp from:to:step:dur`, `--duration`, `--warmup`), `--runs N` (the dry-run cap), `--track firehose|poll`, `--sched-sample` (0 disables step-event subscription — Prometheus stays authoritative), `--inline` (submission-path cost, H6), `--max-inflight`, timeouts, `--out`, `--fail-on-lost`. Credentials from `--key`/`AGENTLOOM_API_KEY`, base from `--api`/`AGENTLOOM_API_URL` (ctl's convention).

**Non-obvious decisions.** (1) **Hand-rolled HDR histogram** over a dependency, to keep the module offline-buildable. (2) **Poll is the terminal authority, firehose is a sampling accelerator** — chosen deliberately because the firehose bounds tracked runs per connection at `MaxTrackedRuns` (2048, terminal-first eviction), so a knee run past ~2k active runs would thrash discovery; the tracker must not depend on it. (3) **Rate/throughput denominator is the actual steady arrival span** (min→max steady submit time), not the configured window, so a `--runs` dry run (whose arrivals end before the configured duration) still reports an honest achieved rate; the offered rate is the configured constant when set. (4) **Fresh `definition_id` per campaign** (register-or-append-version) makes reconciliation exact. (5) **Clock-skew-corrected e2e** removes both skew and delivery lag from the headline latency. (6) **Quiescence requires the *global* queue to drain** — correct for the dedicated `make load-up` stack; on a shared dev stack it reports "not reached" and waits out `--drain-timeout` (documented caveat).

**Tests.** Pure: `hist_test.go` (percentile accuracy within 1%, exact min/max/mean, monotone distribution), `schedule_test.go` (constant/ramp fire counts — incl. the exact integer-nanosecond ramp arithmetic — `--runs` cap, deterministic weighted mix), `pacer_test.go` (**no-coordinated-omission** property + cancel). Machine: `tracker_test.go` (lifecycle → taxonomy, scheduling pairing with retry-claim exclusion, finalize/lost classification). End-to-end: `run_httptest_test.go` (a fake API in poll mode — **DoD-1** complete artifact with percentiles, **DoD-2** achieved rate within ±5% of offered 50/s, **DoD-3** taxonomy sums to the run count) and `loadgen_integration_test.go` (`integration` tag — the real in-process engine + firehose, `linear-10 ×100` in firehose mode: 100 succeeded, 0 lost, 0 non-deliberate DLQ, scheduling samples collected; measured sched p50 14.9ms/p99 57.8ms, e2e p50 249.7ms/p99 377.6ms, rate error 1.1%). All green under `-race`; `golangci-lint` clean. Verified live against the dev compose stack: a 60-run firehose dry run wrote a complete `summary.{json,md}` + CSVs (49 succeeded + 11 `rate_limited` — the dev stack's default submit bucket exercising the taxonomy — 0 lost, 150 scheduling samples, all SLOs PASS).

**Docs.** `docs/load/loadgen.md` (new — usage, the report artifact, the failure taxonomy, caveats); `test/load/README.md` + `docs/load/plan.md` §10 + `docs/README.md` index updated; Makefile `loadgen-build` + `load-dry-run` targets. ROADMAP 19.2 ✅. **Next: 19.3** (baseline campaign & bottleneck identification — ramp each scenario to the knee, capture profiles/`pg_stat_statements`, write `docs/load/findings-baseline.md`).

**Local-setup note.** Nothing to migrate. To drive the generator: boot the dedicated load stack with `make load-up` (a clean, rate-limit-raised, pprof-on environment; give the Docker VM ≥ 12 GiB first), then `AGENTLOOM_API_URL=http://localhost:8080 AGENTLOOM_API_KEY=<key> go run ./cmd/loadgen --scenario linear-10 --runs 100 --rate 20` (or `make load-dry-run`). The report lands under `results/`. Running against the *shared* dev stack works but will 429 above ~10/s and never report quiescence — use `make load-up`. The `integration`-tagged test needs the compose Postgres/Redis (`source .env` first, as usual).

### 19.3 — Baseline campaign & bottleneck identification ✅ (M19.3)

**What it delivered.** The baseline load campaign that executes the 19.1 plan — every scenario ramped to its knee on the pinned `make load-up` stack, with pprof / `pg_stat_statements` / `pg_stat_activity` / Redis / docker-stats / Prometheus evidence captured, and `docs/load/findings-baseline.md` naming the top bottleneck from that evidence. **The headline result: the top binder is H1 (serial per-worker execution), confirmed decisively — the fleet delivers ~45–50 steps/s regardless of scenario shape at K=8, while worker CPU sits at ~2 % utilisation.** Two small additive tooling changes support the campaign (a per-ramp-step breakdown in the loadgen report + an evidence-capture driver script); the campaign also surfaced and fixed a real loadgen accounting bug and a load-corpus gap. **No backend runtime change, no migration, no new config var, no new metric.**

**Tooling (additive).** (1) **`internal/loadgen` ramp-step breakdown** — a pure `rampStepStats` binning the tracked runs by the arrival staircase step their intended fire fell in (`rampsteps.go` + `rampsteps_test.go`), emitted as `summary.json.ramp_steps[]` and a "Ramp steps" table in `summary.md` for ramp campaigns: per step the offered `rate_per_sec`, intended/accepted/terminal counts, `backlog`, succeeded/failed, and e2e p50/p99 — so the knee (e2e p99 diverging per offered rate) is a property of the artifact, cross-checked against the Prometheus `engine_step_scheduling_latency_seconds` series. Reused in 19.6. (2) **`scripts/load-campaign.sh`** (`make load-campaign SCENARIO=… ARGS=…`) — one command = one reproducible campaign + its full evidence bundle in `results/<scenario>-<utc>/`: git SHA / host / resolved compose config, `pg_stat_statements_reset()` at start, background samplers (`docker stats`, `pg_stat_activity` wait events), mid-steady-window pprof capture (worker + API, CPU + heap, rendered to `*.top.txt`), and at the end top-25 `pg_stat_statements` (by time and by calls), Redis `INFO`/`LATENCY`, and Prometheus `query_range` exports over the exact `[campaign_start, arrivals_end]` window for a fixed 12-query list. Curated excerpts committed under `docs/load/baseline/` (`results/` is gitignored).

**The campaign (K=8, mock provider, offline).** Ramp-to-knee per scenario + a K-scaling probe. Every run's integrity held: **0 lost runs, 0 non-deliberate dead letters, full queue quiescence** after each drain.

| scenario | steps/run | knee (runs/s) | fleet steps/s @ knee | worker CPU @ saturation |
|---|---|---|---|---|
| linear-10 | 10 | ~4.5 | ~45 | 2.3 % of the 0.5-core cap |
| fanout-50 | 53 | ~0.9 | ~48 | 6.5 % |
| planner-heavy | 21 | ~2.2 | ~46 | 2.35 % |
| agent-loop | 9 | ~6.0 | ~54 | — |
| mixed | ~21 | ~2.2 | ~45 | 2.65 % |

**The H1 evidence (decisive).** (1) **K-scaling probe** (linear-10 at K=4/8/12): fleet throughput ≈ **5.5 × K steps/s** (23 / 45 / ≥60), while worker CPU stays near-idle at every K (3.7 % / 2.3 % / 1.7 % of the per-worker cap) — throughput scales linearly with replica count, the textbook serialization signature. (2) **Worker pprof at peak saturation** (queue 500+ deep, e2e p99 > 100 s): a 25 s CPU profile sampled **580 ms = 2.3 % utilisation**, dominated by `Syscall6` blocking waits, no engine hot function — the worker is blocked on the (simulated) provider call inside a *serial* claim loop, not compute-bound. (3) **Root cause in code:** `internal/queue/consumer.go` processes each read-batch message serially in a blocking `for` (`c.deliver`), and `cmd/worker` runs exactly **one** `Consumer` per process → fleet step-concurrency = replica count. (4) **Graph-shape-independent:** linear chains, wide fan-outs, planner expansions, and agent loops all hit the same ~45–50 steps/s ceiling.

**Postgres is not the binder at K=8 (H2/H3/H4/H7 not reached / refuted).** `pg_stat_statements` over the full ~9-min linear-10 campaign: the entire top-25 sums to **~43 s of exec over 540 s wall on a 2-CPU Postgres (~8 % of one core)**, every statement sub-millisecond (led by `AllocateEventSeq`, `GetRun`, `step_logs` COPY, `AppendEvent`, `LockRun` — the write-amp ranking for when H1 is lifted). `pg_stat_activity` during fanout-50's 50-way join showed **zero lock waits** (34 idle connections, 1 active query) → **H4 (join-counter hot row) refuted**, H3/H7 not reached. fanout-50 saturating at the *same* steps/s as linear-10 is the clean H4 refutation.

**Selected 19.4 target.** H1 — the top bottleneck. Remediation: **bounded in-worker step concurrency** (a pool of N concurrent claim/execute loops per worker, making fleet concurrency K × N), with an ADR-002/ADR-005 amendment. Quantified goal: raise sustained fleet throughput from **~45 steps/s → ≥ ~180 steps/s** (N≈8) at K=8 while keeping scheduling p99 ≤ 2 s and the correctness suites (5.8 chaos, 13.5 expansion matrix) green. This is **not** in 19.4's default pre-authorised list (batch inserts / LISTEN-NOTIFY / indexes / join-counter) — those target the Postgres binders (H2–H4/H7), which are not yet saturated and move to 19.5's post-fix re-measurement; the findings doc says so explicitly rather than silently widening 19.4.

**Two side findings, both fixed by the campaign.** (1) **loadgen false `run_failed` at high concurrency** — a stale poll/reconcile read (issued before a run finished) could land *after* the fresh terminal firehose event and clobber the run's status back to `running`, so `classOf` read `terminal && status==running` as `run_failed`. The engine was correct throughout (all runs succeeded, 0 DLQ); fixed in `tracker.go` (never overwrite an already-terminal status) with a regression test. (2) **planner-heavy did not expand on the load mock** — the load mock's `default` text response (needed to bound linear-10's chained echo) suppressed the mock's structured plan-echo, so `planner` steps returned prose and dead-lettered on the implicit `json_schema` validator; fixed with a `test/load/mock.json` rule that matches the planner prompt (substring `"schema_version"`) with an empty outcome, making the mock echo the plan verbatim as native structured output. planner-heavy now expands offline.

**SLO ratification.** The plan §3 targets (scheduling p99 ≤ 2 s, 1,000 active runs) are **not met at K=8** — the expected pre-fix state at the H1 ceiling, not a hardware limit (scheduling p50 holds at ~7–10 ms below the knee). Targets retained unchanged; 19.4/19.5 close the gap, 19.6 certifies.

**Tests.** `internal/loadgen` suite green under `-race` (+ the new `TestRampStepStats` / `TestRampStepStatsNonRamp` / `TestTrackerStalePollDoesNotClobberTerminal`); `internal/loadtest` + `internal/llm` green; golangci-lint clean. The campaign artifacts (5 scenario bundles + K-scaling) are committed under `docs/load/baseline/`.

**Docs.** `docs/load/findings-baseline.md` (new — the analysis); `docs/load/baseline/` (new — curated evidence); `docs/load/loadgen.md` (ramp-step breakdown + campaign-script sections); `test/load/README.md` (the mock-rule note); Makefile `load-campaign` target. ROADMAP 19.3 ✅. **Next: 19.4** (remediation #1 — in-worker step concurrency, findings-driven).

**Local-setup note.** The campaign ran against `make load-up` (which replaced the dev stack and rebuilt the images); `make up-app` restores the ordinary dev stack afterward. `test/load/mock.json` gained a rule, so a running load fleet must be **force-recreated** to reload it (`docker compose … up -d --force-recreate worker`) — a plain `up -d` does not restart an unchanged container spec even when a mounted file changes. Reproduce any campaign with `scripts/load-campaign.sh <scenario> <ramp flags>` (commands in the findings doc); give the Docker VM ≥ 12 GiB before a 19.6-scale sustained run.
