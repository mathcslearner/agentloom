...
- [Load-test plan](load/plan.md) — the M19 methodology: SLO targets, scenario definitions, measurement methodology, the resource-pinned environment (`make load-up`), and the pre-registered bottleneck hypotheses. The scenario/definition corpus lives in [`test/load/`](../test/load/README.md); the [load generator](load/loadgen.md) (`cmd/loadgen`) drives it; the [baseline findings](load/findings-baseline.md) (ticket 19.3) name the top bottleneck — **H1, serial per-worker execution** — with pprof/`pg_stat_statements` evidence and the curated bundles under [`load/baseline/`](load/baseline/README.md); certified numbers arrive in `BENCHMARKS.md` (ticket 19.6).
