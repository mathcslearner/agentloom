...
- **Position:** M2 (durable state: Postgres persistence) in progress; 2.1 (Docker Compose dev stack), 2.2 (migration tooling & integration-test harness), 2.3 (ADR-004 & core schema v1), and 2.4 (store layer & transaction helpers) done. **Next ticket: 2.5** (atomic run instantiation).
