...
- **Position:** M2 (durable state: Postgres persistence) in progress; 2.1 (Docker Compose dev stack), 2.2 (migration tooling & integration-test harness), 2.3 (ADR-004 & core schema v1), 2.4 (store layer & transaction helpers), and 2.5 (atomic run instantiation) done. **Next ticket: 2.6** (guarded state transitions / CAS).
