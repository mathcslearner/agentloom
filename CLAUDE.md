...
- **Position:** M3 (queue & lease layer: Redis Streams) in progress — 3.1 (ADR-005) and 3.2 (stream primitives: envelope codec, `XADD` producer, race-safe group bootstrap, introspection — first code in `internal/queue`) complete. **Next ticket: 3.3** (consumer loop with ack/nack semantics).
- **Complete:** M0 foundation (0.1–0.5; one loose end below), all of M1 (workflow definition core: JSON contract per ADR-003, strict codec, validation, graph algorithms + readiness, CEL edge conditions, canonical examples — plus a post-M1 audit/hardening pass), all of M2 (compose stack, migrations + integration harness, ADR-004 schema v1, store layer, atomic run instantiation, guarded CAS transitions — plus a post-M2 audit/hardening pass: seq-after-CAS + uniform run-lock ordering in transitions, app-written `updated_at`, CI integration job boots the compose stack), M3's 3.1 (ADR-005: pointer envelopes with versioning policy, PEL-as-lease with `JUSTID` heartbeats, ACK discipline + takeover rule, crash matrix with per-cell recovery, delayed-delivery ZSET contract, orphan-consumer janitor), and 3.2 (`internal/queue` producer & bootstrap per ADR-005, `RedisConfig`, typed envelope-decode errors, `Stats` depth/PEL introspection).
